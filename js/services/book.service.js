...
function getBookById(bookId) {
  if (!booksDB.length) {
    query()
      .then(books => {
        return Promise.resolve(booksDB.find(book => book.id === bookId))
      })
  }
...
function getNextBookId(bookId) {
  let currBookIdx = getBookIdx(bookId);
  let nextBookIdx = (currBookIdx + 1 === booksDB.length) ? 0 : currBookIdx + 1;
  return Promise.resolve(booksDB[nextBookIdx].id);
}

function getPrevBookId(bookId) {
  let currBookIdx = getBookIdx(bookId);
  let prevBookIdx = (currBookIdx === 0) ? booksDB.length - 1 : currBookIdx - 1;
  return Promise.resolve(booksDB[prevBookIdx].id);
}

function getBookIdx(bookId) {
  return booksDB.findIndex(book => book.id === bookId);
}

function addFeedback(bookId, review) {
  return getBookById(bookId)
    .then(book => {
      // if there aren't any reviews - set array of reviews with current review
...
function addGoogleBook(googleBook) {
  ...
  // get books if refreshed page
  return query()
    .then(books => {
      // search id in array
      let book = books.find(book => book.id === googleBook.id);

      // if id allready exist - tell cmp
      if (book === undefined) {

        // manage book
        let newBook = manageBook(googleBook);
        ...
        return Promise.reject('failur');
      }
    })
}

function manageBook(googleBook) {
  let amount = (googleBook.saleInfo.listPrice) ? googleBook.saleInfo.listPrice.amount : 'N/A';
  let currencyCode = (googleBook.saleInfo.listPrice) ? googleBook.saleInfo.listPrice.currencyCode : 'N/A';
  let isOnSale = (googleBook.saleInfo.saleability === 'FOR_SALE') ? true : false;
  let book =  {
    id: googleBook.id,
    title: googleBook.volumeInfo.title,
    subtitle: googleBook.volumeInfo.subtitle,
    authors: (googleBook.volumeInfo.authors) ? googleBook.volumeInfo.authors : ['AMAMI'],
